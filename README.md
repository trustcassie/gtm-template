# Syrenis Google Tag Manager Consent Mode v2 Integration

**Overview**

The Google Tag Manager template is designed to seamlessly integrate a Syrenis Cookie Banner with Google Consent mode v2. Our Syrenis CMP tag allows you to adjust how your Google tags behave based on the consent status of your users.

**Syrenis CMP Tag Features**
...
**Google Tag Manager Installation Guide**
 Clients should follow the steps below to integrate our Syrenis CMP tag within Google Tag Manager:
...
**Step 2: Add Syrenis's CMP Tag to your Library**
...
- Search and select the "Syrenis CMP" tag.
...
**Step 3. Configure Syrenis CMP Tag**

Now that you have selected the Syrenis CMP tag, it is required that the tag is configured to the Syrenis Cookie Banner by populating the Access Key, detailed in step 3.1. You have the ability to configure this tag to meet your specific requirements however we recommend following the below steps in addition to your custom configurations.
...
The access key field can be located within the Syrenis CMP tag titled "Syrenis Access Key". It is required that this field is populated using the same access key used for the Syrenis Cookie Banner in order for the Syrenis CMP tag to register the session consent.

The Access Key can be obtained in the Syrenis Administration Portal.
...
GTM applies a default state of 'Granted' to all Consent states until a user's consent is received. You can configure default consent based on your regulatory requirements per geographical region in the Syrenis CMP tag by using "Default Consent Settings".
...
- Select Syrenis CMP Tag within your Tags library
...
A firing trigger is required for the Syrenis CMP tag to fire. In addition to the built-in trigger “Consent Initialization - All Pages” you also need to add a custom trigger.
...
Now that we have a Syrenis CMP tag capturing updated user consent against GTM Consent states, we can configure other tags to fire based on the stored consent.
